...
from app.core.auth import get_current_active_user, get_current_active_superuser, is_superuser
...
def get_payment(
    ...
    if payment.user_id != current_user.id and not is_superuser(current_user):
        raise HTTPException(
...
def create_payment(
    *,
    ...
    if payment_in.user_id != current_user.id and not is_superuser(current_user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
...
@router.post("/{payment_id}/upi-ref", response_model=PaymentResponse)
