...
@router.get("/noc")
async def get_noc_list(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_user)
):
    """Get NOC list for the current user."""
    logger.info(f"NOC list requested for user: {current_user.id}")
    
    try:
        # Get user profile and wallet data for NOC generation
        profile = db.query(Profile).filter(Profile.user_id == current_user.id).first()
        income_wallet = db.query(IncomeWallet).filter(IncomeWallet.user_id == current_user.id).first()
        
        # Return basic NOC data structure
        noc_data = {
            "id": "noc_" + current_user.id,
            "user_id": current_user.id,
            "document_url": f"/api/noc/document?user_id={current_user.id}",
        ...
        return [noc_data]  # Return as list to match expected format
        
    except Exception as e:
        logger.error(f"Error getting NOC list: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error getting NOC list: {str(e)}"
        )
