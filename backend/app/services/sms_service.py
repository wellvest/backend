...
class SMSService:
    def __init__(self):
        ...
        # Set up multiple Fast2SMS API URLs to try in order
        # Using both domain names and direct IP addresses to bypass DNS issues
        self.fast2sms_urls = [
            "https://www.fast2sms.com/dev/bulkV2",
            "https://smsapi.fast2sms.com/dev/bulkV2",
            "https://www.fast2sms.com/dev/bulk",
            "http://www.fast2sms.com/dev/bulkV2"  # Try HTTP if HTTPS fails
        ]
    ...
    def _send_sms_fast2sms(self, phone: str, message: str) -> bool:
        ...
        # For OTP messages, use the OTP route which is more reliable
        if "verification code" in message.lower() or "otp" in message.lower():
            ...
            payload = {
                "route": "q",  # Quick route for non-DLT messages
                "message": message,
                "language": "english",
                "flash": 0,
                "numbers": phone,
        ...
        # Try all URLs with retry mechanism
        last_error = None
        ...
        for url in self.fast2sms_urls:
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"Sending SMS via Fast2SMS to {phone} using URL: {url} (attempt {attempt+1}/{self.max_retries})")
                    
                    # Use data parameter instead of json for form-urlencoded content
                    response = requests.request("POST", url, data=payload, headers=headers, timeout=self.timeout)
        ...
        # If we reach here, all attempts with all URLs have failed
        if last_error:
            raise last_error
        else:
            error_msg = "All Fast2SMS URLs failed after multiple attempts"
            logger.error(error_msg)
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail=error_msg
