...
class NotificationService:
    ...
    @staticmethod
    def create_payment_rejected_notification(db: Session, user_id: str, payment_id: str, amount: float, reason: str = None) -> None:
        from app.utils.notification_utils import send_payment_rejected_notification
        send_payment_rejected_notification(db, user_id, payment_id, amount, reason or "No reason provided")
        
    @staticmethod
    def create_interest_notification(db: Session, user_id: str, investment_id: str, amount: float) -> None:
        from app.utils.notification_utils import send_interest_notification
        send_interest_notification(db, user_id, investment_id, amount)
        
    @staticmethod
    def create_investment_completed_notification(db: Session, user_id: str, investment_id: str, amount: float, returns: float) -> None:
        from app.utils.notification_utils import send_investment_completed_notification
        send_investment_completed_notification(db, user_id, investment_id, amount, returns)     
    @staticmethod
