...
class PlanAdmin(BaseModelView, model=Plan):
    ...
    column_list = [Plan.id, Plan.name, Plan.min_amount, Plan.max_amount, Plan.duration_months, Plan.returns_percentage]
    column_searchable_list = [Plan.name]
    column_sortable_list = [Plan.min_amount, Plan.max_amount, Plan.duration_months, Plan.returns_percentage]
    can_create = True
    can_edit = True
    can_delete = True
    can_view_details = True
    
    # Exclude description column that doesn't exist in the database
    form_excluded_columns = ["description"]


class PaymentAdmin(BaseModelView, model=Payment):
    name = "Payment"
    name_plural = "Payments"
    icon = "fa-solid fa-credit-card"
    column_list = [Payment.id, Payment.user_id, Payment.plan_id, Payment.amount, Payment.upi_ref_id, Payment.status, Payment.created_at]
    column_searchable_list = [Payment.upi_ref_id, Payment.status]
    column_sortable_list = [Payment.amount, Payment.created_at, Payment.status]
    can_create = True
    can_edit = True
    can_delete = True
    can_view_details = True
    
    # Add custom form fields for admin notes when approving/rejecting
    form_columns = [Payment.user_id, Payment.plan_id, Payment.amount, Payment.upi_ref_id, Payment.status, Payment.admin_notes]
    
    # Add custom action buttons for approving and rejecting payments
    def row_actions(self, row):
        actions = super().row_actions(row)
        if row.status == PaymentStatus.PENDING:
            actions.append({
                "name": "approve",
                "label": "Approve",
                "url": f"/admin/payment/approve/{row.id}",
                "class": "btn btn-success",
            })
            actions.append({
                "name": "reject",
                "label": "Reject",
                "url": f"/admin/payment/reject/{row.id}",
                "class": "btn btn-danger",
            })
        return actions
    
    # Add custom endpoints for approve/reject actions
    async def on_model_change(self, form, model, is_created, request, is_deleted=False):
        # If status is changed to approved or rejected, handle the payment processing
        if not is_created and not is_deleted and model.status in [PaymentStatus.APPROVED, PaymentStatus.REJECTED]:
            ...
            db = SessionLocal()
            try:
                if model.status == PaymentStatus.APPROVED:
                    PaymentService.approve_payment(db, model.id, model.admin_notes)
                elif model.status == PaymentStatus.REJECTED:
                    PaymentService.reject_payment(db, model.id, model.admin_notes)
            finally:
                db.close()
